...
/// A price level in a limit order book, lock-free on its common paths.
///
/// Add, match (`Gtc` / `Ioc` / `Day`), and cancel take no lock — they run on
/// atomic counters and lock-free structures. A fill-or-kill match is the one
/// exception: it takes a level-exclusive guard across its feasibility check and
/// sweep so it stays all-or-nothing against concurrent mutation, and admissions
/// / updates pay one uncontended shared acquisition (see the `fok_guard` field
/// and [`Self::match_order`] for the full argument — issue #112).
///
/// # Topology
///
...
pub struct PriceLevel {
    ...
    /// atomicity (issue #81). The guarded value is `()`; a poisoned lock (a
    /// holder panicked) is recovered with `into_inner`, since there is no state
    /// to corrupt.
    fok_guard: RwLock<()>,
}
...
impl PriceLevel {
    ...
    /// with `into_inner` because the guarded value is `()` (see the field doc).
    #[inline]
    ...
    pub fn match_order(
        ...
        // the `has_matchable_depth` check: depth committed before it is crossed
        // (reject); depth committed after it is ordered "later" (behind this
        // taker), so resting is correct at the check instant. No guard is needed.
        if taker_kind.is_post_only() && incoming_quantity > 0 {
            if self.has_matchable_depth(taker_order_id) {
                tracing::debug!(
                    taker_order_id = %taker_order_id,
                    incoming_quantity,
                    price = self.price,
...
impl PriceLevel {
    ...
    pub fn update_order(
        ...
        let _fok = self.fok_read();
        self.update_order_inner(update)
    }
