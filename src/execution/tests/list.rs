...
mod tests {
    ...
    fn test_transaction_list_round_trip() {
        ...
        for i in 0..parsed.transactions.len() {
            assert_eq!(parsed.transactions[i].transaction_id, original.transactions[i].transaction_id);
            assert_eq!(parsed.transactions[i].taker_order_id, original.transactions[i].taker_order_id);
            assert_eq!(parsed.transactions[i].maker_order_id, original.transactions[i].maker_order_id);
            assert_eq!(parsed.transactions[i].price, original.transactions[i].price);
            assert_eq!(parsed.transactions[i].quantity, original.transactions[i].quantity);
            assert_eq!(parsed.transactions[i].taker_side, original.transactions[i].taker_side);
            assert_eq!(parsed.transactions[i].timestamp, original.transactions[i].timestamp);
        }
    }
...
mod transaction_list_serialization_tests {
    ...
    fn test_custom_display_format() {
        ...
        // Verificar contenido de las transacciones
        assert!(display_str.contains("transaction_id=12345"));
    ...
    fn test_from_str_invalid_format() {
        // Formato completamente incorrecto
    ...
    fn test_from_str_invalid_transaction() {
        // Transacción inválida dentro de la lista (transaction_id no es un número)
        ...
        // Transacción con formato incorrecto
    ...
    fn test_custom_serialization_round_trip() {
        ...
        // Verificar los campos de las transacciones
    ...
    fn test_from_into_trait_implementations() {
        ...
        // Vec<Transaction> -> TransactionList con From
        let list: TransactionList = transactions.clone().into();
        assert_eq!(list.len(), transactions.len());

        // TransactionList -> Vec<Transaction> con Into
        let list = TransactionList::from_vec(transactions.clone());
        let vec: Vec<Transaction> = list.into();
    ...
    fn test_complex_transaction_list_parsing() {
        // Una lista con más de dos transacciones para probar el parsing de múltiples elementos
